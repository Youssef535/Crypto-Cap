...
const News = () => {
  ...
  const newsAPI = async () => {
    ...
    let { data } = await axios.get(
      'https://newsapi.org/v2/top-headlines?country=us&apiKey=5fca490bd4d54b23a7143e2fbab1dea9'
    );
          ...
          {news.map((news) => (
            <Col key={news.title} md={4}>
              <Card className="bg-dark " style={{ margin: '10px', cursor: 'pointer' }}>
                ...
                <Card.Body>
                  <Card.Title>
                    {news.title && news.title.length > 60 ? news.title.substring(0, 60) + '...' : news.title}
                  </Card.Title>
                  <Card.Text>
                  ...
                  <Button variant="primary" className="link" href={news.url}>
                    Read More
                  </Button>
                </Card.Body>
              </Card>
